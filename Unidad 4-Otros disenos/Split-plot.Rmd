...
He aquí un esquema de un diseño split-plot sencillo:
...
```
Una de las consecuencias de este diseño experimental es que uno de los factores (tipo de suelo en este caso), se convierte en un factor de agrupación de otro factor experimental. A dicha agrupación de un factor experimental dentro de otro se le denomina **anidación**, la cual da origen a un **diseño anidado**. Es importante saber identificar la anidación porque el factor experimental que está anidado (fertilizante) dentro de otro factor será afectado por las fuentes de variabilidad que ocurren dentro del factor de anidación (tipo de suelo). Entonces, el análisis de datos de un diseño anidado requiere de un modelo de efectos mixtos. Debido a que el componente aleatorio de un diseño anidado está relacionado con el factor experimental de anidación, es necesario contar con réplicas verdaderas de este factor.
...
#### Eefecto del tiempo de exposición al tratamiento
...
Como en el ejemplo de Wissinger, hay varias alternativa para este experimento. La primera, era aplicar los tratamientos a grupos diferentes de pacientes, lo que duplicaría el número de voluntarios para el experimento. Aún así existiría el factor de anidación para las mediciones repetidas del efecto de cada tratamiento en el nivel de autoestima. Para eliminar este último nivel de anidación se requeriría consguir más coluntarios a quienes se les midiera el *nivel* de autoestima en los diferentes tiempos. Este diseño incrementaría drásticamente la variabilidad, sin mencionar el gran reto logístico de coordinar un experimento de tal magnitud. Para comprender la estructura de los datos, podemos echar un vistazo a la base de datos:
...
#### Reducción de unidades experimentales

En la primera unidad vimos cómo el diseño experimental puede ayudarnos a solventar problemas logísticos en la sección de [Unidad experimental](/..Unidad 1-Introducción/Unidad-exper.html), particularmente mediante la aplicación secuencial de tratamientos a unidades experimentales como animales para reducir el número de animales requeridos para completar el experimento.
...
Como se mencionó arriba, en los diseños split-plot hay que saber identificar los factores de anidación para así incluirlos adecuadamente en el análisis. Una errónea especificación llevaría a un uso equivocado de los grados de libertad. Comenzaremos por ver el análisis de la base `selfesteem`.

## Análisis de `selfesteem2`

La base de datos mostrada en la tabla \@ref(tab:selfesteem) es la que analizaremos ahora. En la tabla podemos ver que hay seis mediciones de autoestima en cada individuo participante, por lo que, para medir el efecto de tiempo hay cinco grados de libertad, y para medir el efecto del tratamiento hay dos (tres observaciones por tratamiento), y el factor de anidación es el individuo (columna `id`). El formato de la base es ancho por lo que hay que transformarla a largo para el análisis:
...
```{r fig.height=4, fig.width=8, fig.cap="Gráfico de cajas del nivel de autoestima en función del tiempo y la dieta.", fig.align="center"}
...
Con esto podemos ver que sólo hay una combinación de tratamientos que se desvía significativamente de la normalidad. Ahora sí continuemos con el análisis utilizando `aov`. Comenzaremos con ver qué inferiríamos sobre $H_0$ si omitimos la estructura del error (efectos aleatorios):
...
A estas alturas, nos resulta evidente que hay pocas razones para rechazar las hipótesis, nulas, excepto por el efecto de `tratamiento`. Como ya sabemos, los datos de este experimento fueron generados con un diseño split-plot. En este experimento, ambos factores están anidados en los individuos, de modo que en `aov` debemos especificarlo en los efectos aleatorios como `Error(id/(tiempo * tratamiento))`. Esta especificación hará que los efectos de ambos factores experimentales se estimen para cada individuo participante y que por lo tanto se pierdan muchos grados de libertad en los residuales:

```{r message = 4}
m2 <- aov(auto ~ tiempo * tratamiento + Error(id/(tiempo * tratamiento)), auto.l)
summary(m2)
```
Como podemos ver, las conclusiones serían completamente diferentes entre un método y otro. Cuando incluimos correctamente los efectos aleatorios, rechazamos todas las hipótesis nulas, particularmente sobre la interacción `tiempo:tratamiento`, con lo que concluimos que el efecto del tratamiento con dieta depende del tiempo que ésta dure.
