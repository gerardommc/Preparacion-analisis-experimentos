...
- Aquí, parcela = bloque = tratamiento
...
3. Los diseños anidados **siempre** requieren de la presencia de efectos aleatorios
...
1. En Wissinger et al. (1996)
      ...
      1.1. Tratamiento de densidad anidado en régimen
      
      1.2. Régimen se confunde con bloque
      
      1.3. Aunque el régimen se confunde con bloque, efecto aleatorio se especifica por bloque

```{r echo = TRUE, eval=FALSE}
m <- aov(Reclutamiento ~ Densidad * Regimen + Error(Densidad/Bloque),
              data = Wissinger)
```
...
```{r echo=T, eval=F}
m1 <- aov(auto ~ tratamiento * tiempo + Error(id/tratamiento/tiempo),
          selfesteem2)
```
