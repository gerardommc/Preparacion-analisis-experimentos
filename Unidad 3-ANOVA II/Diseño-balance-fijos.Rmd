...
Y efectivamente, los valores de $P$ para `supp` son diferentes que cuando especificamos el modelo como `supp * dose`, pues la base de comparación para `supp` ahora es `dose`. También podemos apreciar que `supp : dose` es igual, y esto se debe a que tiene la misma base de comparación. El valor *idéndico* de $P$ para `dose` se debe a que el número más pequeño que **R** (la computadora) puede mostrar es $2 \times 10^{-16}$.

```{r}
modelo.4 <- aov(len ~ dose + supp, data = datos)
summary(modelo.4)
```
