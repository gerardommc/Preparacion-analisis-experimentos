...
Para hacer esta prueba en **R** nos se requiere de ningún paquete especial, y se usa la función `kruskal.test`. El primer argumento que se da a la función es la fórmula del modelo del mismo como que con `aov`y `lm`, `y ~ x`y el segundo es la base de datos que contiene a $y$ y $x$. Igual que antes simularemos una base de datos sencilla, pero la analizaremos con ANOVA y Kruskal-Wallis.
...
Primero verifiquemos que los datos no tengan una distribución normal:
...
sugieren que hay diferencias significativas entre tratamientos ($P < 0.05$), mientras que los resutlados de de Kruskal-Wallis:
...
Y efectivamente, la línea roja de la primera gráfica no es horizontal, y los puntos de la segunda gráfica se limitan a la línea punteada, con lo que concluimos que ANOVA es inadecuado para rechazar la hipótesis nula de no diferencia entre tratamientos.
