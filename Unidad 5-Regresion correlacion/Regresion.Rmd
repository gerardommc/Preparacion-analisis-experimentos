...
\begin{equation}
      \beta_0 = \frac{ \sum y_i \cdot \sum x_i^2 – \sum x \cdot \sum x_iy_i} {n \cdot \sum x_i^2 – \left(\sum x_i \right)^2} (\#eq:beta0)
\end{equation}

\begin{equation}
      \beta_1 = \frac{n\cdot \sum x_i y_i - \sum x_i \cdot \sum y_i}{n \cdot \sum x_i^2 - \left( \sum x_i \right)^2} (\#eq:beta1)
\end{equation}
...
\begin{equation}
   SS_{Error} = \sum y_i^2 - \beta_0 \sum y_i - \beta_1 \sum x_i y_i (\#eq:SS-err)
\end{equation}

\begin{equation}
   SS_{Total} = \sum y_i^2 - \frac{\left( \sum y_i \right)^2}{n} (\#eq:SS-tot)
\end{equation}
...
Para rechazar $H_0$ y $H_1$, entonces, necesitamos estimar la probabilidad de que estas tengan un valor de $0$, utilizando el estadístico $t$ ($\beta_0$ y $\beta_1$ tienen una distribución $t$, similar a la normal). Para estimar este último necesitamos calcular las siguientes cantidades:
